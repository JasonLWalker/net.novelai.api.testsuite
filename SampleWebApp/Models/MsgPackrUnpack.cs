 ...
 * 
 * Can loosely use the RFC7049 specification found at https://datatracker.ietf.org/doc/html/rfc7049
 * as a reference, though CBOR is not quite the same as MessagePack
 *
 */
...
public class MsgPackrUnpack : IMsgUnpackerOptions
{
    ...
    public class CurrentStructuresModel : Dictionary<int, object?>
    {
        public Dictionary<int, object?>? RestoreStructures { get; set; }
    ...
    protected Dictionary<int, object?> CurrentStructures => _currentStructures;
    ...
    // Extension 0x42
    private class Ext66 : MsgPackrExtension { public Ext66() : base(66) => AddUnpackHandler((byte[]? bytes) => null); }


    // Extension 0x62
    protected Ext98Model? _ext98Model;
    protected class Ext98Model
    {
        public string[] Strings { get; } = new string[2];
        public int Position0 { get; set; }
        public int Position1 { get; set; }
    ...
    private class Ext98 : MsgPackrExtension
    {
        ...
        {
            AddUnpackHandler((byte[]? args = null) =>
            {
                if (args is null)
    ...
    // Extension 0x65
    private class Ext101 : MsgPackrExtension { public Ext101() : base(101) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0x69
    private class Ext105 : MsgPackrExtension { public Ext105() : base(105) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0x70
    private class Ext112 : MsgPackrExtension { public Ext112() : base(112) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0x73
    private class Ext115 : MsgPackrExtension { public Ext115() : base(115) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0x74
    private class Ext116 : MsgPackrExtension { public Ext116() : base(116) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0x78
    private class Ext120 : MsgPackrExtension { public Ext120() : base(120) => AddUnpackHandler((byte[]? bytes) => null); }
    // Extension 0xff
    private class Ext255 : MsgPackrExtension { public Ext255() : base(255) => AddUnpackHandler((byte[]? bytes) => null); }
    #endregion
    ...
    public virtual object? Read()
    {
        ...
        if (token < 0xa0) // Less than 0xa0 Represents primitives or arrays
        {
            if (token < 0x80) // Less than 0x80 represents primitives
            {
                ...
                {
                    if (_currentStructures.ContainsKey((token ?? 0) & 0x3F))
                    {
                        var structure = _currentStructures[(token ?? 0) & 0x3F]; // Apply mask
                ...
                case 0xc1:
                    if (bundledStrings != null)
                    {
                        ...
                        else
                        {
                            ...
                            return bundledStrings.Strings[0].Substring(start, len);
                        }
                        ...
                        if (_currentExtensions.ContainsKey((byte)value))
                        {
                            ...
                            else
                            {
                                if (extension.Unpack != null)
                                {
                                    ...
                                    return extension.Unpack(ReadBytes(position, 1));
                                }
                            }
                        }
                        else
    ...
    public virtual MsgPackrStructureReadCallback? CreateSecondByteReader (int? firstId, int? read0) {
        /*
        return function() {
    ...
    protected virtual object? RecordDefinition(int id, int? highByte = null)
    {
        ...
        var existingStructure = (CurrentStructuresModel?)_currentStructures.FirstOrDefault(o => o.Key == id).Value;
        ...
        {
           	(_currentStructures.RestoreStructures ?? (_currentStructures.RestoreStructures = []))[id] = existingStructure;
    ...
    protected object? SaveState(object? callback)
    {
        // Todo: Implement the following:
        /*
